...
type EditorPanel = 'link' | 'image' | 'youtube';

type EditableMedia = 'image' | 'youtube';
...
@Component({
  ...
  imports: [AssetLibrary, AsyncPipe, I18nPipe, Icon],
  providers: [
    {
...
export class MarkdownEditor implements AfterViewInit, OnDestroy, ControlValueAccessor {
  ...
  private readonly imageLibrary = viewChild(AssetLibrary);
  ...
  protected readonly editingMedia = signal<EditableMedia | null>(null);
  protected readonly linkUrl = signal('');
  protected readonly imageAssetId = signal('');
  protected readonly imageAlt = signal('');
  protected readonly imageAlignment = signal<ContentMediaAlignment>('center');
  protected readonly imageSize = signal<ContentMediaSize>('large');
  protected readonly youtubeValue = signal('');
  protected readonly youtubeAlignment = signal<ContentMediaAlignment>('center');
  protected readonly youtubeSize = signal<ContentMediaSize>('large');
  ...
  protected readonly selectedImage = computed(
    () => this.availableAssets().find((asset) => asset.id === this.imageAssetId()) ?? null,
  );
    ...
    const editor = new Editor({
      ...
      editorProps: {
        attributes: {
          ...
          role: 'textbox',
          'aria-multiline': 'true',
          'aria-label': this.label() ?? 'Markdown',
        },
      },
      onUpdate: ({ editor: currentEditor }) => {
        this.value = currentEditor.getMarkdown();
        this.onChange(this.value);
        this.bumpRevision();
      },
      onSelectionUpdate: ({ editor: currentEditor }) => {
        this.bumpRevision();
        this.closeMediaPanelIfSelectionChanged(currentEditor);
      },
      onBlur: () => {
        this.onTouched();
      },
  ...
  writeValue(value: string | null): void {
    ...
    if (editor && editor.getMarkdown() !== this.value) {
      editor.commands.setContent(this.value, {
        contentType: 'markdown',
        emitUpdate: false,
      });
    }
  ...
  protected isHeadingActive(level: 2 | 3): boolean {
    this.revision();
  ...
  protected isImageSelected(): boolean {
    ...
    return this.selectedNodeAttributes(this.editor(), 'assetImage') !== null;
  }

  protected isYoutubeSelected(): boolean {
    this.revision();

    return this.selectedNodeAttributes(this.editor(), 'youtubeEmbed') !== null;
  }
  ...
  protected openPanel(panel: EditorPanel): void {
    ...
    if (panel === 'link') {
      ...
      this.linkUrl.set(typeof href === 'string' ? href : '');
    } else if (panel === 'image') {
  ...
  protected applyLink(): void {
    const href = this.normalizeLink(this.linkUrl());

    if (!href) {
      this.errorKey.set('admin.markdown.invalidLink');
  ...
  protected selectImageAsset(asset: Asset): void {
    this.imageAssetId.set(asset.id);
    this.errorKey.set(null);
  }

  protected addImageAsset(asset: Asset): void {
    this.imageAssetId.set(asset.id);
    this.assetUploaded.emit(asset);
  }

  protected removeImageAsset(assetId: string): void {
  ...
  protected setImageAlt(event: Event): void {
    this.imageAlt.set(this.readControlValue(event));
  }
  ...
  protected saveImage(): void {
    ...
    if (!assetId) {
      this.errorKey.set('admin.markdown.imageRequired');
    ...
    if (this.editingMedia() === 'image' && this.selectedNodeAttributes(editor, 'assetImage')) {
      editor.chain().focus().updateAttributes('assetImage', attrs).run();
    } else {
      editor
        .chain()
        .focus()
  ...
  protected setYoutubeValue(event: Event): void {
    this.youtubeValue.set(this.readControlValue(event));
  }

  protected saveYoutube(): void {
    const videoId = this.extractYoutubeId(this.youtubeValue());
    const editor = this.editor();

    if (!videoId) {
      this.errorKey.set('admin.markdown.invalidYoutube');
    ...
    if (this.editingMedia() === 'youtube' && this.selectedNodeAttributes(editor, 'youtubeEmbed')) {
      editor.chain().focus().updateAttributes('youtubeEmbed', attrs).run();
    } else {
      editor
        .chain()
        .focus()
  ...
  protected removeSelectedMedia(): void {
    ...
    const nodeName = editingMedia === 'image' ? 'assetImage' : 'youtubeEmbed';

    if (!this.selectedNodeAttributes(editor, nodeName)) {
      return;
    }
  ...
  protected setImageAlignment(alignment: ContentMediaAlignment): void {
    this.imageAlignment.set(alignment);
  }

  protected setImageSize(size: ContentMediaSize): void {
    this.imageSize.set(size);
  }

  protected setYoutubeAlignment(alignment: ContentMediaAlignment): void {
    this.youtubeAlignment.set(alignment);
  }

  protected setYoutubeSize(size: ContentMediaSize): void {
    this.youtubeSize.set(size);
  }

  private closeMediaPanelIfSelectionChanged(editor: Editor): void {
    const editingMedia = this.editingMedia();
  ...
  private loadSelectedImage(): boolean {
    const attrs = this.selectedNodeAttributes(this.editor(), 'assetImage');

    if (!attrs) {
      return false;
    }

    this.imageAssetId.set(this.readStringAttribute(attrs, 'assetId'));
    this.imageAlt.set(this.readStringAttribute(attrs, 'alt'));
    this.imageAlignment.set(this.readMediaAlignment(attrs['alignment']));
    this.imageSize.set(this.readMediaSize(attrs['size']));
  ...
  private loadSelectedYoutube(): boolean {
    const attrs = this.selectedNodeAttributes(this.editor(), 'youtubeEmbed');

    if (!attrs) {
      return false;
    }

    this.youtubeValue.set(this.readStringAttribute(attrs, 'videoId'));
    this.youtubeAlignment.set(this.readMediaAlignment(attrs['alignment']));
    this.youtubeSize.set(this.readMediaSize(attrs['size']));
  ...
  private readStringAttribute(attrs: Record<string, unknown>, name: string): string {
    const value = attrs[name];

    return typeof value === 'string' ? value : '';
  }

  private readMediaAlignment(value: unknown): ContentMediaAlignment {
    return value === 'left' || value === 'right' || value === 'center' ? value : 'center';
  }

  private readMediaSize(value: unknown): ContentMediaSize {
  ...
  private extractYoutubeId(value: string): string | null {
    ...
    try {
      ...
      if (host === 'youtu.be') {
        return this.validateYoutubeId(url.pathname.split('/').filter(Boolean)[0]);
      }
      ...
      if (url.pathname === '/watch') {
        return this.validateYoutubeId(url.searchParams.get('v'));
      }
  ...
  private validateYoutubeId(value: string | null | undefined): string | null {
